...
describe('ValueConverter', () => {
    describe('toLabels', () => {
        it('should throw an error when input is not of string type', () => {
            const input = 123;
            expect(() => Result.unwrap(ValueConverter.toLabels(input))).toThrowError(
                'Invalid value type, expected string.',
            );
        });
        ...
        it('should return array of one string if there are no whitespaces', () => {
            const input = 'abc';
            const expected = ['abc'];
            expect(Result.unwrap(ValueConverter.toLabels(input))).toEqual(expected);
        });
    ...
    describe('toCallstack', () => {
        it('should throw an error when input is not of string type', () => {
            const input = 123;
            expect(() => Result.unwrap(ValueConverter.toCallstack(input))).toThrowError(
                'Invalid value type, expected string.',
            );
        });
        ...
        it('should return empty "frame" array if input is empty', () => {
            const input = '{ "frame": [] }';
            const expected: string[] = [];
            expect(Result.unwrap(ValueConverter.toCallstack(input))).toEqual(expected);
        });
